...
$nome =  $_POST['nome-mar'];
$telefone =  $_POST['telefone-mar'];
$cpf =  $_POST['cpf-mar'];
$email =  $_POST['email-mar'];
$endereco =  $_POST['endereco-mar'];
...
if($antigo != $cpf){
$query = $pdo->query("SELECT * FROM marketing where cpf = '$cpf' ");
...
if($antigo2 != $email){
$query = $pdo->query("SELECT * FROM marketing where email = '$email' ");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_reg = @count($res);
if($total_reg > 0){
...
$res = $pdo->prepare("INSERT INTO marketing SET nome = :nome, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone");
$res2 = $pdo->prepare("INSERT INTO usuarios SET nome = :nome, cpf = :cpf, email = :email, senha = :senha, nivel = :nivel");
$res2->bindValue(":senha", '123');
$res2->bindValue(":nivel", 'marketing');
}else{

  $res = $pdo->prepare("UPDATE marketing SET nome = :nome, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone WHERE id = $id");
